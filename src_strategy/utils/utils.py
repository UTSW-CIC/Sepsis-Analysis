...
def cast_cols(df: pl.DataFrame, schema: dict):
    ...
    for col, rule in schema.items():
        ...
        if isinstance(rule, tuple) and rule[0] == "datetime":
            ...
            exprs.append(pl.col(col).str.strptime(pl.Date, rule[1]))
        else:
...
def extract_sys_dia_from_flowsheets(df_flowsheets: pl.DataFrame,
    ...
    val_col = bp_config.raw_val_col

    df_bp = df_flowsheets.filter(pl.col(event_grouper_col) == bp_grouper_val).with_columns(
...
def extract_arterial_blood_pressure_mean_from_flowsheets(df_flowsheets: pl.DataFrame,
